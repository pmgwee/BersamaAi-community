...
## Key facts
- **LLM = `grok-4.6` at `xhigh` reasoning effort, via OpenCode Go**'s Responses API (`https://opencode.ai/zen/go/v1` — base URL only; the `openai` SDK appends `/responses`). Config is provider-NEUTRAL: `LLM_API_KEY` / `LLM_BASE_URL` / `LLM_MODEL` / `LLM_REASONING_EFFORT`, and the provider lives behind ONE module, `bersama-ai-pipeline/pipeline/llm.py` (the bot has its own 4-var block in `bot.py`). OpenCode Go requires this API client to send its own `User-Agent` and a stable `x-opencode-session` conversation ID; `llm.py` supplies both centrally. Migrated off Z.ai/GLM on 2026-08-24, off `gpt-5.6-luna` on 2026-09-14 — swapping again = edit `llm.py` + the env vars, not the feature code.
